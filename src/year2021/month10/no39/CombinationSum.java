...
public class CombinationSum {
    ...
    private static void backtrack(int[] candidates, int target, List<List<Integer>> ans, List<Integer> list, int pos, int curSum) {
        if (curSum == target) {
        ...
        for (int i = pos; i < candidates.length; i++) {
            ...
            if (nextSum > target) {  // 剪枝
                break;
            }
            list.add(candidates[i]);
            backtrack(candidates, target, ans, list, i, nextSum);
