...
public class CopyListWithRandomPointer {
    ...
    private static Node dfs(Node head, Map<Node, Node> nodeMap) {
        if (head == null) {
            return null;
        }
        if (nodeMap.containsKey(head)) {
        ...
        // 新建 node 之后需在递归前 put 入 map，否则会有死循环
        nodeMap.put(head, node);
        node.next = dfs(head.next, nodeMap);
        node.random = dfs(head.random, nodeMap);
        return node;
    }
