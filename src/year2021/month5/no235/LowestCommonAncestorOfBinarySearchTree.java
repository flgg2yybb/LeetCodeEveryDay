...
public class LowestCommonAncestorOfBinarySearchTree {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(lowestCommonAncestor(root, p2, q2));
    }
    ...
    private static TreeNode dfs(TreeNode root, TreeNode max, TreeNode min) {
        if (max.val < root.val) {
            // max 和 min 同为 root 的左子节点
            return dfs(root.left, max, min);
        }
        if (min.val > root.val) {
            // max 和 min 同为 root 的右子节点
            return dfs(root.right, max, min);
        }
        // max.val >= root.val && min.val <= root.val
        // 又因为 max.val != min.val
        // 故 min, max比为root本身或者其子节点，且不可能同为 root 的左子节点或柚子节点
        return root;
    }
